...
## Общая задача: имеются комнаты, которые связаны между собой трубами. Комнаты и связи между ними описаны на карте. Есть генератор карт, а также некоторые примеры карт находятся в папке ```maps```. В карте сначала приводится перечисление названий комнат с координатами, а далее приводятся связи между комнатами. В комнате, перед которой команда ```##start```, находятся ```N``` муравьев. ```N``` написано в первой строчке файла карты. Последняя комната имеет перед собой команду ```##end```. Любые другие строчки-комментарии начинаются с ```#```. Пример карты: 
```
##start
abc 100 140
abcd 110 150
